...
function generateNavLinks(linkTexts, targetElementId) {
    ...
    if (targetElement) {
        targetElement.innerHTML = linkTexts.map((text) => {
            return `<li class="text-emerald-700 hover:text-emerald-900 transition-all duration-300 ease-in-out hover:translate-x-1 font-semibold hover:bg-none">
                        <a class="flex items-center gap-1 font-semibold" href="#${text.toLowerCase()}">
                            ${text}
